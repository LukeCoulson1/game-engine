...
void CollisionEditorWindow::renderCollisionEditor() {
    ...
    if (ImGui::SliderFloat("Scale", &m_visualizationScale, 1.0f, 8.0f, "%.1fx")) {
        // Scale changed manually - user override
    }
    ImGui::Checkbox("Show Grid", &m_showGrid);
    
    ImGui::Text("Zoom: Use mouse wheel to zoom in/out");
    ImGui::Text("Paint: Left-click and drag to paint collision areas");
...
void CollisionEditorWindow::renderSpriteWithCollision() {
    ...
    // Create child window for the visual editor with NoScrollWithMouse to prevent interference
    if (ImGui::BeginChild("CollisionVisualizer", ImVec2(0, 0), true, 
                         ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse)) {
        
        // Handle mouse wheel zoom first (before other input handling)
        if (ImGui::IsWindowHovered()) {
            ...
            if (io.MouseWheel != 0.0f) {
                ...
                editorWidth = spriteWidth * m_visualizationScale;
                editorHeight = spriteHeight * m_visualizationScale;
                m_editorSize = ImVec2(editorWidth, editorHeight);
            }
        }
...
void CollisionEditorWindow::handlePaintbrushTool() {
    ...
    if (mouseInSprite) {
        // Create an invisible button over the sprite area to capture mouse input
